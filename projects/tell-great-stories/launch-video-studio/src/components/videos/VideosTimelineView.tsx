...
import type { StoryboardScene, VideoClip } from '@/types/project';
...
function TimelineScene({
  ...
  const approvedClip = clips.find(c => c.isApproved && c.status === 'completed');
  const hasCompletedClips = clips.some(c => c.status === 'completed');
  ...
  return (
    ...
    >
      ...
      >
        {/* Thumbnail - show video if approved, otherwise show source image */}
        <div className="aspect-video bg-zinc-100 relative">
          {approvedClip ? (
            ...
            <>
              <video
                src={clips.find(c => c.status === 'completed')?.videoUrl}
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="w-10 h-10 rounded-full bg-black/30 flex items-center justify-center">
                  <PlayIcon className="w-5 h-5 text-white/70 ml-0.5" />
                </div>
              </div>
            </>
          ) : isGenerating ? (
          ...
          {/* Clip count badge */}
          {hasCompletedClips && !approvedClip && (
            <div className="absolute bottom-1.5 left-1.5 bg-black/70 text-white text-[10px] px-1.5 py-0.5 rounded">
              {clips.filter(c => c.status === 'completed').length} clip{clips.filter(c => c.status === 'completed').length > 1 ? 's' : ''}
            </div>
          )}
        </div>
...
export interface VideosTimelineViewProps {
...
/**
 * Horizontal timeline view for videos phase
 */
export function VideosTimelineView({
  scenes,
  clips,
  sourceImages,
  selectedSceneId,
  ...
  const approvedCount = scenes.filter(scene =>
    clips.some(clip => clip.sceneId === scene.id && clip.isApproved && clip.status === 'completed')
  ).length;
          ...
          <div className="flex gap-6">
            {scenes.map((scene, index) => (
              <TimelineScene
                key={scene.id}
                scene={scene}
                clips={clips.filter(clip => clip.sceneId === scene.id)}
                sourceImage={sourceImages[scene.id] || null}
