...
interface GeneratedStoryboard {
  scenes: Array<{
    ...
    transition: string;
    notes?: string;
    logoAppropriate?: boolean;
    logoPlacementNotes?: string;
  }>;
  pacingNotes?: string;
  ...
  "scenes": [
    {
      ...
      "logoAppropriate": true,
      "logoPlacementNotes": "Where/how the logo should appear (if logoAppropriate is true)"
    }
  ],
...
For each scene, determine if the company logo should appear (logoAppropriate: true/false).
Logo-appropriate scenes include:
- Product UI/screen shots (logo on the interface)
- Call-to-action scenes (logo as brand reinforcement)
- Brand reveal moments
- Scenes with branded materials or devices
- Closing/outro scenes

Do NOT include logo in:
- Establishing shots or abstract visuals
- Emotional reaction shots focused on people
- Fast-paced action sequences
- Metaphorical or symbolic imagery

When logoAppropriate is true, provide logoPlacementNotes describing where/how it should appear (e.g., "on laptop screen", "as watermark in corner", "on product packaging").
...
Create a compelling ${targetDuration}-second video storyboard. Each scene should show emotion through action, not just tell. The total duration of all scenes should equal approximately ${targetDuration} seconds.`,
    ...
    const scenes: StoryboardScene[] = parsedStoryboard.scenes.map((scene, index) => ({
      ...
      notes: scene.notes,
      logoAppropriate: scene.logoAppropriate ?? false,
      logoPlacementNotes: scene.logoPlacementNotes,
    }));
